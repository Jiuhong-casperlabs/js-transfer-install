...
const main = async () => {
  ...
  const [deploy, deployRes] = await client.getDeploy("595F274E5e83FD3df897c95dAc144af52215Fd566cBA1CD1576d95dd2DE1cbAC");
  const toPubKey = deploy.session.getArgByName("target");
  console.log("toPubKey:", toPubKey);
  console.log("deployRes", deployRes);
  console.log("toPubKey", JSON.stringify(Buffer.from(toPubKey.value()).toString("hex"),null,4));};
